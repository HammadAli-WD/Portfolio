import React from "react";
import { Timeline, Events, UrlButton, ImageEvent } from "@merc/react-timeline";
import Accordion from "react-bootstrap/Accordion";
import Card from "react-bootstrap/Card";
import Image from "react-bootstrap/Image";
import "./projects-timeline.styles.css";

const TimeLine = () => {
  return (
    <div id="projects">
      <h1 className="pt-3 text-center font-details-b pb-3">PROJECTS</h1>
      <Timeline>
        <Events>
          {/* Project: expats in germany BE */}
          <ImageEvent
            date="11/01/2020"
            className="text-center"
            text="Node.js Express API Expatriates In Germany"
            src="https://scontent-dus1-1.xx.fbcdn.net/v/t1.0-9/125951845_3464996786901414_4634422103179623787_o.jpg?_nc_cat=104&ccb=2&_nc_sid=730e14&_nc_ohc=ZTTQeexMIgIAX9Cou2e&_nc_ht=scontent-dus1-1.xx&oh=07b324c39b8f401a97bf0a060aa5ffa8&oe=5FDA92C2"
            alt="API-Expats"
          >
            <div className="d-flex justify-content-between flex-column mt-1">
              <div>
                <Accordion>
                  <Card>
                    <Accordion.Toggle
                      as={Card.Header}
                      eventKey="0"
                      className="p-2 text-center accordian-main"
                    >
                      PROJECT DETAILS
                    </Accordion.Toggle>

                    <Accordion.Collapse eventKey="0" className="text-left">
                      <Card.Body>
                        <strong>Description:</strong> This is a API build in Nodejs and Express with MongoDB-Atlas storage for helpful data on expatriates in Germany
                        <hr />
                        <strong>Features:</strong>
                        <ul className="list-styles pt-1">
                          <li>Weather data of main cities in Germany with forecast of seven days</li>
                          <li>Covid data of Germany with CFR</li>
                          <li>Chat routes for sharing information among members</li>
                          <li>OAuth with facebook and linkedIn</li>
                          <li>Socket-io for real time communication</li>
                        </ul>
                        <hr />
                        <strong>Tech used:</strong>
                        <ul>
                          <li>
                            <span className="p-2">

                              <Image
                                src="https://miro.medium.com/max/640/1*-ivYkzeuYJedPKdEdfnNlg.png"
                                alt="MongoDB"
                                rounded
                                className="image-style m-1"
                              ></Image>{" "}
                              MongoDB
                            </span>
                          </li>
                          <li>
                            <span className="p-2">
                              <Image
                                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT_0Uu824N-jZhYSbpzi1YIoS-3cNkRhbkBPw&usqp=CAU"
                                alt="Node.js"
                                rounded
                                className="image-style m-1"
                              ></Image>{" "}
                              Node.js
                            </span>
                          </li>
                          <li>
                            <span className="p-2">
                              <Image
                                src="https://expressjs.com/images/express-facebook-share.png"
                                alt="Express"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              Express
                            </span>
                          </li>
                          <li>
                            <span className="p-2">
                              <Image
                                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTb7-hcPyysyISeZXrYcksyddecTgznyyBzWQ&usqp=CAU"
                                alt="socket.io"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              socket.io
                            </span>
                          </li>
                          <li>
                            <span className="p-2">
                              <Image
                                src="https://hidenobu-tokuda.com/wp-content/uploads/2019/05/heroku-logo-solid-gradient.png"
                                alt="Heroku"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              Heroku
                            </span>
                          </li>

                        </ul>
                      </Card.Body>
                    </Accordion.Collapse>
                  </Card>
                </Accordion>
              </div>
              <div className="d-flex justify-content-between flex-nowrap text-center">
                <UrlButton
                  href="https://apiexpatsingermany.herokuapp.com/news/hackerNews"
                  target="_blank"
                >
                  SEE LIVE
                </UrlButton>
                <UrlButton
                  href="https://github.com/HammadAli-WD/expatsGermany-BE"
                  target="_blank"
                >
                  SOURCE CODE
                </UrlButton>
              </div>
            </div>
          </ImageEvent>


          {/* Project: FrontEnd Exaptriates In Germany */}
          <ImageEvent
            date="11/09/2020"
            className="text-center"
            text="Front-End Project For Expatriates"
            src="https://scontent-dus1-1.xx.fbcdn.net/v/t1.0-9/126185055_3465001626900930_8391058147239461293_o.jpg?_nc_cat=103&ccb=2&_nc_sid=730e14&_nc_ohc=TWPA7jePdJQAX9KdWia&_nc_ht=scontent-dus1-1.xx&oh=a24c2ad413af0c70bdc1cc7c04dd7f41&oe=5FDC258D"
            alt="Front-End Project For Expatriates"
          >
            <div className="d-flex justify-content-between flex-column mt-1">
              <div>
                <Accordion>
                  <Card>
                    <Accordion.Toggle
                      as={Card.Header}
                      eventKey="0"
                      className="p-2 text-center accordian-main"
                    >
                      PROJECT DETAILS
                    </Accordion.Toggle>

                    <Accordion.Collapse eventKey="0" className="text-left">
                      <Card.Body>
                        <strong>Description:</strong> This app gives information regarding different processes and information about the Germany. The User can approach the
                        weather data of main cities, and news in Germany. The information about the COVID data is important especially for the mobility to Germany and therefore provided
                        in this app for the Germany. The users can discuss the issues and resources through the group chat provided in the app.
                        <hr />
                        <strong>Features:</strong>
                        <ul className="list-styles pt-1">
                          <li>React App with mostly functional componenets</li>
                          <li>Real-time API calls from backend API</li>
                          <li>Bing News on a daily basis fetching from Rapid API</li>
                          <li>Styling by using styled-components</li>
                        </ul>
                        <hr />
                        <strong>Tech used:</strong>
                        <ul>
                          <li>
                            <span className="p-2">
                              <Image
                                src="data:image/svg+xml;base64,"
                                alt="React"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              React
                            </span>
                          </li>
                          <li>
                            <span className="p-2">
                              <Image
                                src="https://d2eip9sf3oo6c2.cloudfront.net/tags/images/000/000/321/full/reactrouter.png"
                                alt="React Router"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              React Router
                            </span>
                          </li>
                          <li>
                            <span className="p-2">
                              <Image
                                src="data:image/svg+xml;base64,"
                                alt="React Bootstrap"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              React Bootstrap
                            </span>
                          </li>
                          <li>
                            <span className="p-2">
                              <Image
                                src="https://styled-components.com/logo.png"
                                alt="styled-components"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              styled-components
                            </span>
                          </li>
                          <li>
                            <span className="p-2">
                              <Image
                                src="https://hidenobu-tokuda.com/wp-content/uploads/2019/05/heroku-logo-solid-gradient.png"
                                alt="Heroku"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              Heroku
                            </span>
                          </li>
                        </ul>
                      </Card.Body>
                    </Accordion.Collapse>
                  </Card>
                </Accordion>
              </div>
              <div className="d-flex justify-content-between flex-nowrap text-center">
                <UrlButton
                  href="https://expatsingermany.herokuapp.com/"
                  target="_blank"
                >
                  SEE LIVE
                </UrlButton>
                <UrlButton
                  href="https://github.com/HammadAli-WD/expatsGermanyFE"
                  target="_blank"
                >
                  SOURCE CODE
                </UrlButton>

              </div>
            </div>
          </ImageEvent>

          {/* Project: My Portfolio */}
          <ImageEvent
            date="11/18/2020"
            className="text-center"
            text="Portfolio"
            src="https://scontent-dus1-1.xx.fbcdn.net/v/t1.0-9/126145744_3465005526900540_1578087433939689662_o.jpg?_nc_cat=105&ccb=2&_nc_sid=730e14&_nc_ohc=Xh59s_QqoDsAX-XOOPA&_nc_ht=scontent-dus1-1.xx&oh=e247adbbedfd8e1e07c56027464a53ca&oe=5FD9EB60"
            alt="Portfolio"
          >
            <div className="d-flex justify-content-between flex-column mt-1">
              <div>
                <Accordion>
                  <Card>
                    <Accordion.Toggle
                      as={Card.Header}
                      eventKey="0"
                      className="p-2 text-center accordian-main"
                    >
                      PROJECT DETAILS
                    </Accordion.Toggle>

                    <Accordion.Collapse eventKey="0" className="text-left">
                      <Card.Body>
                        <strong>Description:</strong> This is my first portfolio in React js, which shows brief introduction about my skills and education.
                        <hr />
                        <strong>Features:</strong>
                        <ul className="list-styles pt-1">
                          <li>Function based application</li>
                          <li>Gives detail about my education and skills</li>

                        </ul>
                        <hr />
                        <strong>Tech used:</strong>
                        <ul>
                          <li>
                            <span className="p-2">
                              <Image
                                src="data:image/svg+xml;base64,"
                                alt="React"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              React
                            </span>
                          </li>
                          <li>
                            <span className="p-2">
                              <Image
                                src="data:image/svg+xml;base64,"
                                alt="React Bootstrap"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              React Bootstrap
                            </span>
                          </li>
                          <li>
                            <span className="p-2">
                              <Image
                                src="https://styled-components.com/logo.png"
                                alt="styled-components"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              styled-components
                            </span>
                          </li>
                          <li>
                            <span className="p-2">
                              <Image
                                src="https://hidenobu-tokuda.com/wp-content/uploads/2019/05/heroku-logo-solid-gradient.png"
                                alt="Heroku"
                                rounded
                                className="image-style1 m-1"
                              ></Image>{" "}
                              Heroku
                            </span>
                          </li>
                        </ul>
                      </Card.Body>
                    </Accordion.Collapse>
                  </Card>
                </Accordion>
              </div>
              <div className="d-flex justify-content-between flex-nowrap text-center">
              </div>
            </div>
            <UrlButton
              href="https://competent-liskov-49a579.netlify.app/"
              target="_blank"
            >
              SEE LIVE
                </UrlButton>
            <UrlButton
              href="https://github.com/HammadAli-WD/portfolio.github.io"
              target="_blank"
            >
              SOURCE CODE
                </UrlButton>
          </ImageEvent>



        </Events>
      </Timeline>
    </div>
  );
};

export default TimeLine;
